...
const Navbar = ({ setCurrentLang, currentLang }: Langs) => {
 ...
 const navigation: Navigation[] = [
  { name: t("navbar.blog"), href: "/blogposts", key: "blog" },
  { name: t("navbar.join"), href: "/volunteersignup", key: "join" },
 ...
 return (
  <nav className="bg-gradient-to-br from-green-500 via-green-600 to-green-600 shadow-2xl lg:sticky top-0 z-50">
   <div className="flex h-24 items-center justify-between px-4">
    {/* Logo */}
    <a href="./">
     <img alt="プライドランドロゴ" src="/logo.png" className="h-24 p-2 m-0" />
    </a>
    {/* desktop view links */}
       ...
       <MenuItem key={lang}>
        {({ active }) => (
         <button
          ...
          className={`w-full flex px-4 py-2 text-sm text-white ${
           active ? "text-secondary bg-green-300" : ""
          }`}>
          <img
           src={langs[lang].flag}
           alt={`${lang} flag`}
